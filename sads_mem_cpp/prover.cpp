...
UINT update_leaf(ULONG nodeid) {
	UINT *p_value = NULL;
	ULONG *p_nodeid = (ULONG *)malloc(sizeof(ULONG));

	*p_nodeid = nodeid;
	p_value = (UINT *)g_hash_table_lookup(leaf_value_table, p_nodeid);

	SVector init_label = get_initial_label(TRUE);
	char *init_label_buffer = encode_vector(init_label);

	if(p_value == NULL) {
		DEBUG_TRACE(("update_leaf: add a new leaf(%llu)\n", nodeid));

		p_value = (UINT *)malloc(sizeof(UINT));
		*p_value = 1;

		g_hash_table_insert (leaf_value_table, p_nodeid, p_value);

		g_hash_table_insert (label_table, p_nodeid, init_label_buffer);

	}
	else {
		DEBUG_TRACE(("update_leaf: update a leaf(%llu)\n", nodeid));

		char *label_buffer = (char *)g_hash_table_lookup(label_table, p_nodeid);
		SVector label = decode_vector_buffer(label_buffer, LABEL_BUFFER_LEN);

		label = label + init_label;
		free(label_buffer);
		g_hash_table_insert (label_table, p_nodeid, encode_vector(label));

		*p_value = *p_value + 1;
		g_hash_table_insert (leaf_value_table, p_nodeid,	p_value);
	}

	init_label.resize(0,0);
...
SVector update_partial_label(ULONG nodeid, ULONG wrt_nodeid) {
	...
	if(wrt_nodeid & ((ULONG)1 << (bit_shift_count-1))) {
		partial_digest = R * update_partial_label((nodeid<<1)+(ULONG)1, wrt_nodeid);
	}
	/* Left child */
	else {
		partial_digest = L * update_partial_label(nodeid<<1, wrt_nodeid);
	}
...
void update_node_label(ULONG nodeid, SVector partial_label) {
	...
	*p_nodeid = nodeid;
	...
	label_buffer = (char *)g_hash_table_lookup(label_table, p_nodeid);

	if(label_buffer == NULL) {
		//printf("new label:\n");
		g_hash_table_insert(label_table, p_nodeid, encode_vector(partial_label));
	}
	else {
...
MembershipProof *process_membership_query(ULONG nodeid) {
	...
	while(proof_nodeid_list) {
		...
		label_buffer = (char *)g_hash_table_lookup(label_table, g_memdup(&curr_nodeid, sizeof(curr_nodeid)));
...
#if 0
    printf("check!!!!!!!!!!!!!!!!!!\n");
    ULONG curr_nodeid = 4469227523;
    SVector *label = (SVector *)g_hash_table_lookup(label_table, g_memdup(&curr_nodeid, sizeof(curr_nodeid)));
